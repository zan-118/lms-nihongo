    ...
    <div className="relative min-h-screen bg-background text-foreground flex flex-col md:flex-row overflow-x-hidden w-full transition-colors duration-300">
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${!isExamPage ? 'md:pl-72' : ''} transition-all duration-500`}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {!isExamPage && (
              <div className="w-full px-4 md:px-8 lg:px-12 pt-6 flex items-center gap-4 z-20 relative animate-in fade-in slide-in-from-top-4 duration-500">
                <Button 
                  onClick={() => router.back()}
                  variant="ghost" 
                  className="w-10 h-10 p-0 shrink-0 rounded-xl bg-card border border-border hover:bg-primary/10 hover:border-primary/30 text-muted-foreground hover:text-primary transition-all shadow-sm flex items-center justify-center"
                  aria-label="Kembali"
                >
                  <ChevronLeft size={16} />
                </Button>
                
                <nav className="flex items-center flex-wrap gap-2 text-[10px] md:text-xs font-black uppercase tracking-[0.2em] text-muted-foreground bg-card/40 backdrop-blur-md px-4 py-2 rounded-xl border border-border/50 shadow-sm">
