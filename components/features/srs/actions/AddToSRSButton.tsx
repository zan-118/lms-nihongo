...
export default function AddToSRSButton({ wordId }: { wordId: string }) {
  ...
  if (isAdded) {
    return (
      <button
        disabled
        className="p-3 bg-success/10 border border-success/30 text-success rounded-xl transition-all cursor-default flex items-center justify-center relative group shadow-[inset_0_0_10px_rgba(34,197,94,0.1)]"
      >
        <Check size={18} />
        <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-cyber-bg text-xs font-bold px-3 py-1 rounded-lg border border-success/30 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
