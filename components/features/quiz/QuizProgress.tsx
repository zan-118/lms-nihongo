...
export function QuizProgress({ current, total, color = "bg-primary", indicatorClassName = "" }: QuizProgressProps) {
  ...
  return (
    <div className="w-full h-2 md:h-3 bg-white/[0.03] dark:bg-white/[0.05] rounded-full overflow-hidden relative border border-white/5 neo-inset">
      <motion.div
        initial={{ width: 0 }}
      ...
      >
        ...
        <div className="absolute right-0 top-0 bottom-0 w-1 bg-white shadow-[0_0_15px_rgba(255,255,255,0.8)]" />
      </motion.div>
    </div>
  );
}
