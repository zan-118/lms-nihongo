...
function ShareContent() {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 text-center">
        <Card className="p-12 max-w-md w-full neo-card rounded-[2.5rem] border-destructive/20 bg-card">
          <Skull size={60} className="text-destructive mx-auto mb-6 opacity-20" />
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4 md:p-8 relative overflow-hidden">
      {/* Background Decor */}
      <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] blur-[120px] rounded-full pointer-events-none opacity-10 ${data.passed ? 'bg-success' : 'bg-destructive'}`} />
        ...
        <Card className="p-8 md:p-16 text-center neo-card rounded-[3.5rem] border border-border bg-card/80 backdrop-blur-xl shadow-2xl relative overflow-hidden">
           ...
           <div className="mb-12">
              <motion.div 
                ...
                className={`w-24 h-24 mx-auto neo-inset rounded-[2rem] flex items-center justify-center mb-8 border border-border ${data.passed ? 'text-success' : 'text-destructive'}`}
              >
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-12">
              <div className="p-8 bg-muted/30 border border-border rounded-3xl neo-inset flex flex-col items-center">
                 ...
                 <span className={`text-4xl font-black font-mono ${data.passed ? 'text-success' : 'text-destructive'}`}>
           ...
           <div className="pt-8 border-t border-border">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                 ...
                 <Button 
                    ...
                    variant="ghost" 
                    className="h-14 px-10 border border-border bg-background/5 hover:bg-background/10 text-foreground font-black uppercase tracking-widest text-xs rounded-2xl transition-all"
                 >
